...
/*   Updated: 2024/10/01 19:46:48 by ael-maaz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strdup_alt(const char *s1)
{
	...
	len = ft_strlen_alt(s1);
	buffer = (char *)malloc((len + 1) * sizeof(char));
	if (!buffer)
		return (NULL);
	if (len == 0)
	{
		ft_bzero_alt(buffer, 1);
		return (buffer);
	}
	ft_memmove_alt(buffer, s1, len + 1);
	return (buffer);
}
...
char	*ft_substr_alt(char const *s,  int start, int len)
{
	...
	if (!s)
		return (NULL);
	if (start >= ft_strlen_alt(s))
		return (ft_strdup_alt(""));
	else if (len > (ft_strlen_alt(s) - start))
		len = ft_strlen_alt(s) - start;
...
int	ft_atoi_alt(char *s, int *error)
{
	...
	i = 0;
	if (ft_fcmp_alt(s, "-0") == 0 || ft_fcmp_alt(s, "+0") == 0 || ft_fcmp_alt(s, "0") == 0)
		return (0);
	num = 0;
	sign = 1;
