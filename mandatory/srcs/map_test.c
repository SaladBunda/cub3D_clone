...
/*   Updated: 2024/10/01 19:55:42 by ael-maaz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	(void) line;
	if(ft_fcmp_alt(flag,"F")== 0)
	{
		x->floor.str = ft_strdup_alt(line[1]);
		x->floor.count++;
	}
	if(ft_fcmp_alt(flag,"C")== 0)
	{
		x->ceiling.str = ft_strdup_alt(line[1]);
		x->ceiling.count++;
	}
	if(ft_fcmp_alt(flag,"NO")== 0)
	{
		x->no.str = ft_strdup_alt(line[1]);
		x->no.count++;
	}
	if(ft_fcmp_alt(flag,"SO")== 0)
	{
		x->so.str = ft_strdup_alt(line[1]);
		x->so.count++;
	}
	if(ft_fcmp_alt(flag,"WE")== 0)
	{
		x->we.str = ft_strdup_alt(line[1]);
		x->we.count++;
	}
	if(ft_fcmp_alt(flag,"EA")== 0)
	{
		x->ea.str = ft_strdup_alt(line[1]);
		x->ea.count++;
...
int test_colors(char **line, char *flag, t_map_data *x)
{
	...
	int error = 0;
	int num;
	if(ft_fcmp_alt(flag, "F") == 0 || ft_fcmp_alt(flag, "C") == 0)
	{
...
int open_textures(char **line, char *flag, t_map_data *x)
{
	(void) x;
	int fd;
	if(ft_fcmp_alt(flag, "NO") == 0 || ft_fcmp_alt(flag, "SO") == 0 || ft_fcmp_alt(flag, "WE") == 0 || ft_fcmp_alt(flag, "EA") == 0)
	{
...
int test_line(char **line, t_map_data *x,int *state)
{
	...
	while(line[i])
		i++;
	if(ft_fcmp_alt(line[0], "F") == 0 || ft_fcmp_alt(line[0],"C") == 0 || ft_fcmp_alt(line[0],"NO") == 0|| ft_fcmp_alt(line[0],"SO") == 0|| ft_fcmp_alt(line[0],"EA") == 0|| ft_fcmp_alt(line[0],"WE") == 0)
	{
		if(i != 2)
...
int test_textures(t_data *data, int *index)
{
	char **line;
	t_map_data x;
	int state = 0;
...
int copy_map(t_data *data, int i)
{
	...
	while(data->file[j])
	{
		data->map[l++] = ft_strdup_alt(data->file[j]);
		j++;
	}
	data->map[l] = NULL;
	data->h_map = l;
	...
	while(data->map[i])
	{
		if(ft_strlen_alt(data->map[i]) > max)
			max = ft_strlen_alt(data->map[i]);
		i++;
	}
	data->w_map = max;
...
int parsing_test(t_data *data)
{
	...
	while(i < data->h_map)
	{
		copy[i] = ft_strdup_alt(data->map[i]);
		i++;
	}
	copy[i] = NULL;
	i = 0;
